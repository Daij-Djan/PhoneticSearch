...
//  Metaphone2.m
//
//https://stephenhaunts.com/2014/03/18/phonetic-string-matching-metaphone/
//
...
{
    public static class Metaphone2
    {
        public static string metaphone2Code(this string self)
        {
            return new Metaphone2Implementation().CreateToken(self);
        }
    }

    class Metaphone2Implementation
    {
        ...
        public string CreateToken(string word)
        {
            ...
            if (strippedNonLetters.Length >= 2)
            {
                strippedNonLetters.ToString().Substring(0, 2);
            }

            if (strippedNonLetters[0] == 'X')
            {
                strippedNonLetters[0] = 'S';
            }
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                if (ProcessVowelIfAtBeginning(c, i, metaphoneKey))
                {
                    continue;
                }
...
(strippedNonLetters[i + 2] == 'O' && strippedNonLetters[i + 2] == 'A'))
            ...
            {
                if (!(i > 0 && strippedNonLetters[i - 1] == 'T'))
                {
