...
- (NSString *)soundexCode {
    ...
    [self.class initSoundexCharSetsIfNeeded];
    ...
    if([self length] > 0)
    {
        ...
        NSUInteger indx, soundValue, previousSoundValue = 0;
        ...
        for( indx = 1; indx < [workingString length]; ++indx )
        {
            ...
            if( soundValue > 0 && soundValue != previousSoundValue )
                [soundexStr appendString:[NSString stringWithFormat:@"%lu", (unsigned long)soundValue]];
...
+ (void)initSoundexCharSetsIfNeeded
{
